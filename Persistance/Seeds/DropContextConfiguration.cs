        ...
        protected override void Seed(PersistanceContext context)
        {
            using (DbContextTransaction dbContextTransaction = context.Database.BeginTransaction(System.Data.IsolationLevel.Serializable))
            {
                ...
                context.Movies.AddOrUpdate(m => m.Name,
                       new Movie { Id = 1, Name = "Shrek!", DirectorName = "Spike Lee", ArrivalDate = DateTime.Now.AddDays(-15),ReleaseDate = DateTime.Now.AddDays(-15), Created = DateTime.Now, CreatedBy = "seed", InStock = 5,GenreId=1, Genre = genres.SingleOrDefault(c => c.Id == 1) },
                       new Movie { Id = 2, Name = "Tarzan", DirectorName = "Spike Lee", ArrivalDate = DateTime.Now.AddDays(-15), ReleaseDate = DateTime.Now.AddDays(-10), Created = DateTime.Now, CreatedBy = "seed", InStock = 5, GenreId = 2, Genre = genres.SingleOrDefault(c => c.Id == 2) },
                       new Movie { Id = 3, Name = "Caliman", DirectorName = "Steven Spielberg", ArrivalDate = DateTime.Now.AddDays(-15), ReleaseDate = DateTime.Now.AddDays(-5), Created = DateTime.Now, CreatedBy = "seed", InStock = 5, GenreId = 3, Genre = genres.SingleOrDefault(c => c.Id == 3) },
                       new Movie { Id = 4, Name = "Independence day", DirectorName = "James Cameron", ArrivalDate = DateTime.Now.AddDays(-15), ReleaseDate = DateTime.Now.AddDays(-1), Created = DateTime.Now, CreatedBy = "seed", InStock = 5, GenreId = 4, Genre = genres.SingleOrDefault(c => c.Id == 4) });
